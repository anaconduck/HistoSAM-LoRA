...
def update_ema_variables(
    model_student: nn.Module, model_teacher: nn.Module, alpha: float
):

    with torch.no_grad():
        for param_student, param_teacher in zip(
            model_student.parameters(), model_teacher.parameters()
        ):
            if param_student.requires_grad:
                param_teacher.data.mul_(alpha).add_(
                    param_student.data, alpha=1.0 - alpha
                )
...
def run_iterative_self_training(
    ...
    model_student = build_histo_sam_lora(
        num_classes=3, lora_rank=16, decoder_type="carafe"
    )
    if initial_checkpoint and Path(initial_checkpoint).exists():
        print(
            f"[INFO] Loading initial pre-trained checkpoint from {initial_checkpoint}"
        )
    ...
    for r in range(1, num_rounds + 1):
        ...
        cur_threshold = initial_threshold - progress * (
        ...
        pseudo_ds = PseudoLabeledDataset(
            image_paths, prev_pseudo_dir, img_size=img_size
        )
        if len(pseudo_ds) == 0:
        ...
        train_loader = DataLoader(
            pseudo_ds, batch_size=batch_size, shuffle=True, drop_last=False
        )

        trainable_params = [p for p in model_student.parameters() if p.requires_grad]
        optimizer = torch.optim.AdamW(trainable_params, lr=lr, weight_decay=1e-4)
        criterion = ConfidenceWeightedLoss(
            threshold=cur_threshold, soft_weighting=False
        )
        ...
        print(
            f"[CONVERGENCE] Label Shift Delta vs Round {r-1}: {delta*100:.2f}% (Tolerance: {convergence_tol*100:.2f}%)"
        )
        ...
        if train_stats["avg_loss"] < best_loss:
            ...
            torch.save(
                {"model_state_dict": model_teacher.state_dict(), "history": history},
                best_path,
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Iterative Self-Training for Liver Histopathology"
    )
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="Initial pre-trained checkpoint"
    )
