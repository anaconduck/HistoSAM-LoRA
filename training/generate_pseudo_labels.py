...
def generate_pseudo_labels(
    ...
    print(
        f"[INFO] Generating pseudo-labels for {total_images} images on device {device}..."
    )
    ...
    for idx in range(total_images):
        ...
        if (idx + 1) % 20 == 0 or (idx + 1) == total_images:
            print(
                f"[{idx+1}/{total_images}] Saved: {stem} (Mean Conf: {conf_np.mean():.4f})"
            )

    avg_conf = float(np.mean(conf_stats))
    total_pixels = sum(class_counts.values())
    class_dist = {
        c: f"{(count / total_pixels) * 100:.2f}%" for c, count in class_counts.items()
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Pseudo-Labels and Confidence Maps"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/liver_primary/pseudo_labels",
    ...
    parser.add_argument(
        ...
        type=str,
        default="20",
        help="Filter magnification (e.g. '20', or 'all')",
    )
    parser.add_argument(
    ...
    dataset = HistopathologyDataset(
        image_files, img_size=args.img_size, magnification_filter=mag_filter
    )
    ...
    if args.checkpoint and Path(args.checkpoint).exists():
        ...
        print(
            "[WARN] No checkpoint provided or not found; running with initialization weights."
        )
